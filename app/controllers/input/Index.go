...
func GetInputPage(c *gin.Context) {
	var page,size int
	...
	data, total, _ := services.GetInputPages(input, order+" "+sort, page, size)
	var last = 1
	if total%size == 0 {
		last = total / size
	} else {
...
func SaveInput(c *gin.Context) {
	...
	if isExist {
		err = services.ModifyInput(lichv.IntVal(id), input)
		if err != nil {
			c.JSON(200,gin.H{
				"state":4001,
				"message":err.Error(),
		...
		id, err = services.AddInput(input)
		if err != nil{
			c.JSON(200,gin.H{
				"state":4002,
				"message":err.Error(),
...
func DelInput(c *gin.Context) {
	...
	if isExist {
		err = services.DeleteInput(map[string]interface{}{"id": lichv.IntVal(id)})
		if err != nil {
			c.JSON(200, gin.H{
				"state":   4001,
				"message": err.Error(),
		...
		length := len(input)
		if length > 0{
			err = services.DeleteInput(input)
			if err != nil {
				c.JSON(200, gin.H{
					"state":   4002,
					"message": "删除错误",
