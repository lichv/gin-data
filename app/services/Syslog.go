...
func LogFromContext(c *gin.Context,target ,operation interface{},input interface{})(int,error){
	...
	user := claims.Code
	return models.AddSyslog(map[string]interface{}{"user": user, "user_agent": userAgent, "token": token, "ip": ip, "target": lichv.Strval(target), "operation": lichv.Strval(operation),"input":lichv.Strval(input), "time": time.Now().Unix(), "flag": 1, "state": 1})
}