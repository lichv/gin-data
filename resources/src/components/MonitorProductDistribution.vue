<template>
	<div :id="id" :style="{ width:'1560px',height:'300px' }"></div>
</template>
<script>
	import { watch } from 'vue'
	import api from '../api';
	export default {
		name: 'ElMonitorProductDistribution',
		data(){
			return{
		...
		methods:{
			...
			getData:function(){
				let _this = this;
				api.getProductDistribution.send(this.form)
				.then(result => {
					...
					if (result.state==2000) {
						...
						_this.data = data
						_this.initChartPie(_this.id,{title:"产品分布",name:"产品分布",data:data})
					}
				})
				.catch(e => {
					console.log(e)
